...
> 配套：`docs/设计/技术方案.md`、`docs/设计/UI架构细化方案.md`、`docs/规范/AI工具合规保证规范.md`。
...
> **状态校注（2026-09-18）**：本节（§9）与 §2.3 的修订均出自 2026-09-17 的源码核查，
> 其中的**行号锚点会随代码演进失效**，请以"文件名 + 符号名"为准重新定位。

---

## 10. 落地路径（P0–P5）

> **本节由原《AI协同审核GUI落地清单.md》（2026-09-17 一次性源码核查快照）合并而来，该文档已删除。**
> 删除原因：它是一次性核查产物，全部证据为"文件:行号"锚点（如 `gate.rs:1327-1363`），
> 任何一次代码改动后即失效，会持续输出错误信息；且其 B1–B3 结论已并入本文 §9、
> P0–P5 处置已并入本节。合并时锚点一律改为**文件名 + 符号名**（不写行号）。
> 需要原始清单可从 git 历史取回（`git show HEAD~1:AI协同审核GUI落地清单.md`）。

### 10.0 通用约束与 MVP 裁剪

- `core` **永远零依赖**（`core/Cargo.toml` 不得出现 `[dependencies]`），IPC 只能用 `std`；
- 脚本改动必须 **sh / ps1 双端同步**（BOM、fail-closed 语义一致）；
- 每阶段出口：`cargo test --workspace` + `cargo clippy --workspace --all-targets -- -D warnings`
  + `cargo fmt --all --check` 全绿，外加 `python scripts/verify_gate.py` 全场景通过。

**建议首期只做 MVP**：`P0（结构化）+ 单实例互斥 + 请求桥 + GUI 启动即聚焦`，
**结果回传全部走"文件兜底通道"**（AI 自跑 `gate_check` 感知）——这一环本来就是零新增且最稳的。
双向 IPC 与后台监听线程（完整版 P4）列为后续，首期工作量减半，且"多 AI 不重复弹窗 +
人类批准后 AI 自然解封"已经成立。

### 10.1 P0 —— 结构化拦截原因（硬前置，无此项后面全部落不了）

| 项 | 内容 | 位置（文件名 + 符号） |
| --- | --- | --- |
| 目标 | 拦截结论变成**机器可读**：是否需人工决策 / 哪个需求 / 哪一段 | — |
| 改动 1 | 拦截脚本各拦截分支输出标记：`REQ_GUARD_BLOCK_KIND=<no_requirement\|need_approval\|need_resolve\|not_installed>`、`REQ_GUARD_REQ=<需求ID>`、`REQ_GUARD_STEP=<首个未通过段>` | `core/src/gate.rs` 的 `HOOK_SH` / `HOOK_PS1`（双端同步） |
| 改动 2 | `GateVerdict::Block` 增加结构化字段（`kind`/`req`/`step`），由 `gate_check` 解析标记；**禁止匹配中文文案** | `core/src/gate.rs`（`GateVerdict`、`gate_check`） |
| 改动 3 | **需求 ID 口径**：脚本里的"活跃需求"是**文件名**，须输出 `GATE:HEAD id=` 的需求 ID | `core/src/gate.rs`（`HOOK_SH` 取活跃需求处） |
| 验收（可证伪） | ①四种拦截场景各给各的 `kind`；②故意改写中文提示文案，**解析结果不变**（证明没依赖文案）；③文件名场景输出 `REQ-001` 而非 `REQ-001-xxx.md`；④ps1 在 PowerShell 5.1 下同样输出（BOM 回归） | — |
| 风险 | 脚本改坏 = 全局拦截失效（fail-closed 会**全拦**），必须跑 `scripts/verify_gate.py` 全场景 | — |

### 10.2 P1 —— 环境事实：`ci` 判定（解 D2）

`core/src/ui_mode.rs` 的 `EnvFacts` 增加 `ci`（`CI` / `GITHUB_ACTIONS` / CNB 等），
并新增可单测的"是否允许自动弹窗"判定函数；决策表单测覆盖
`ci × ssh × display × windows/macos` 组合矩阵，CI 下判定为"不弹窗"。

### 10.3 P2 —— 单实例锁 + IPC（解 B2 / D3，核心新增）

- 新增 `core::instance`（零依赖）；锁文件路径复用 `core/src/token.rs` 的 `guard_dir()` 约定；
- **原子抢锁**：`OpenOptions::new().create_new(true)`；锁文件内容 = `pid` + `port` + 心跳时间戳；
- **传输必须是回环 TCP**（`127.0.0.1`，`std::net`）——零依赖下唯一跨平台 IPC；
  `bind("127.0.0.1:0")` 取**动态端口**写入锁文件，不固定端口；
- **僵尸锁回收用心跳**（定时刷新时间戳，超时视为死进程），不依赖查 pid 存活；
- 协议用单行 JSON，**复用现有零依赖解析器 `core/src/json.rs`**；
- 验收：①两进程并发抢锁 → 恰好一个成功；②心跳过期后锁可回收；③协议往返解析单测；
  ④`core/Cargo.toml` **仍无 `[dependencies]`**。

### 10.4 P3 —— 请求桥 CLI（解 D1 / B3）

- `cli/src/cli.rs` 的 `Action::Ui` 扩展 `--request <需求ID>` / `--step` / `--requester` / `--probe`；
- `cli/src/main.rs` 的 `run_ui` 分流：`--probe` 打印结构化探测结果退出；`--request` 先过准入
  （P0 的 `kind` + P1 的 `ci` + display），再抢锁或转 client；
- **★ 启动 GUI 前显式清除 `REQ_GUARD_AI_CTX`**（解 B3）——否则人类在 GUI 里点批准会被审批锁拒绝；
- 降级：无 display → TUI 或打印人工指引；CI → 打印结构化 block 并 `exit 1`（AI 停下等人）；
- **安全代价（须写明）**：清除 `AI_CTX` 后，方案 A 对"GUI 启动路径"失去区分力 →
  **生产环境必须启用方案 B（令牌）或方案 C**，不能只靠方案 A（见 `docs/规范/AI工具合规保证规范.md` §4.4）。

### 10.5 P4 —— GUI 侧接管（解 D4）

| 范围 | 内容 | 位置 |
| --- | --- | --- |
| MVP | `run()` 接受"聚焦目标"（需求 ID + 段），启动后直接定位到该需求该段 | `gui/src/lib.rs`（`run`） |
| 完整版 | 后台线程 `accept` + `mpsc` 推给 App；App 维护**审核请求注册表**（key = 需求 ID + 段，去重），渲染"待审核请求"区 | `gui/src/app.rs` |
| 完整版 | 人类决策落盘后 → 重跑 `gate_check` → 向注册 requester 回推结果；断连忽略（文件兜底） | — |

验收：①两个 AI 先后请求同一需求 → **只一个窗口**；②人类批准后两 AI 自跑 `check` 均通过；
③GUI 真机点击验证。

### 10.6 P5 —— 收口

- **文档**：更新 `docs/设计/技术方案.md` 与 `docs/规范/AI工具合规保证规范.md`
  （新增能力、B3 的安全代价、GUI 审批仍受"锁"约束）；
- **校验**：单实例锁文件是**运行时产物**，不进 `install --verify`；
  如未来要校验，须先确认其平台中立性（沿用 `marker_of()` 取词干的原则）；
...
方案**可以落地**，但顺序不能颠倒：**先 P0 把拦截原因结构化**（否则整套触发逻辑没有输入），
再建单实例与请求桥（注意 IPC 只能是回环 TCP），最后接 GUI（记得清 `REQ_GUARD_AI_CTX`，否则功能自废）。
首期建议只做 MVP，把结果回传交给已有的文件兜底通道。
